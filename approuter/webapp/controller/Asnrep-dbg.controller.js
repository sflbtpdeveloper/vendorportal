sap.ui.define([
    ...
    return BaseController.extend("zmmsubcontract.controller.Asnrep", {
        ...
        onDownloadExcel: function () {
            debugger;
            // Get the table
            var oTable = this.byId("asnTable"); // Replace with your table ID
            ...
            // Check if aData is an object and extract values
            if (aData && typeof aData === 'object') {
                aData = Object.values(aData); // Convert the object to an array
            }

            // Remove __metadata from each item in aData
            aData = aData.map(function (item) {
                // Create a new object without the __metadata property
                const { __metadata, ...rest } = item;
                return rest;
            });

            // Check if aData is an array and has elements
            if (!Array.isArray(aData) || aData.length === 0) {
                console.error("Data is either not an array or is empty");
                return;
            }

            // Ensure the first item is an object before proceeding
            if (typeof aData[0] !== 'object' || aData[0] === null) {
                console.error("The first element is not an object");
                return;
            }

            // Get the visible column headers
            const visibleColumns = oTable.getColumns().filter(column => column.getVisible());

            // Filter aData to only include properties that match visible column IDs
            const filteredData = aData.map(row => {
                const filteredRow = {};
                visibleColumns.forEach(column => {
                    const fieldName = column.getSortProperty();
            ...
            var csvContent = this._convertToCSV(filteredData, visibleColumns);

            // Trigger the download
            this._downloadCSV(csvContent, 'ASN-Report.csv');
        },

        _convertToCSV: function (data, visibleColumns) {
            // Extract headers using the column labels from the table
            const headers = visibleColumns.map(column => {
                var oLabel = column.getLabel(); // Get the header control
                return oLabel ? `"${oLabel.getText()}"` : ''; // Safely get the text or fallback
            }).join(',') + '\n';

            // Map each row of data to a CSV format, including only visible fields
            const rows = data.map(row => {
                return visibleColumns.map(column => {
                    const fieldName = column.getSortProperty(); // Get the property name based on the column ID
                    let value = row[fieldName]; // Access the value in the row based on the field name

                    // Handle date formatting for Dcdate
                    if (fieldName === 'Dcdate' && value) {
                        const date = new Date(value);
                        // Format the date to DD/MM/YYYY
                        // value = `${('0' + date.getDate()).slice(-2)}/${('0' + (date.getMonth() + 1)).slice(-2)}/${date.getFullYear()}`;
                    }

                    // Handle potential commas and new lines in values
                    return `"${value !== undefined ? value : ''}"`;
                }).join(','); // Join values with a comma for CSV format
